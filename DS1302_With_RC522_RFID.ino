...
void setup() {
  ...
  Serial.begin(9600); // dsetup duong truyen
  // set up thoi gian
  rtc.setDOW(FRIDAY);
  rtc.setTime(16,0, 0);
  rtc.setDate(31, 5, 2019);
}


byte AAuthorized[6] = {0x62, 0x99, 0x7C, 0x6F, 0xFF, 0xFF};//Mã thẻ đã lập trình để nhận diện
...
void loop() {
  ...
  if (status == MI_OK) {
    ...
    { 
      //printSerial(serial);
      Serial.print("Ma the dung");
      Serial.print("le van te");
